...
func TestController_calculateProfit(t *testing.T) {
	...
	t.Run("limit order", func(t *testing.T) {
		...
		_, err = controller.OrderLimit(model.SideTypeBuy, "BTCUSDT", 1, 1000)
		require.NoError(t, err)

		// should execute previous order
		wallet.OnCandle(model.Candle{Time: time.Now(), Symbol: "BTCUSDT", High: 1000, Close: 1000})

		sellOrder, err := controller.OrderLimit(model.SideTypeSell, "BTCUSDT", 1, 2000)
		require.NoError(t, err)
	...
	t.Run("oco order limit maker", func(t *testing.T) {
		...
		sellOrder, err := controller.OrderOCO(model.SideTypeSell, "BTCUSDT", 1, 2000, 500, 500)
		require.NoError(t, err)
	...
	t.Run("oco stop sell", func(t *testing.T) {
		...
		_, err = controller.OrderLimit(model.SideTypeBuy, "BTCUSDT", 0.5, 1000)
		require.NoError(t, err)

		_, err = controller.OrderLimit(model.SideTypeBuy, "BTCUSDT", 1.5, 1000)
		require.NoError(t, err)

		// should execute previous order
		wallet.OnCandle(model.Candle{Time: time.Now(), Symbol: "BTCUSDT", Close: 1000, Low: 1000})

		_, err = controller.OrderMarket(model.SideTypeBuy, "BTCUSDT", 1.0)
		require.NoError(t, err)
